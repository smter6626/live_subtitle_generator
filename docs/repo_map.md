...
| Branch | `main` |
| Structural baseline | `3a369e55456430a2a5e0b9a55279f1dc1dc2f939` (`fix: improve current model readability`) |
...
| `test_model_manager_ui_contract.py` | `ui_app.py` Model Manager orchestration and model presentation contracts | Guards that current summaries stay concise with full-path tooltips, combo/log/table keep full presentation where intended, download work remains in the background worker, finished states re-enable/refresh, and UI does not bypass integrity by accepting an existing final file. |
...
| --- | --- |
| `docs/product_polish_static.md`, `docs/product_polish_runtime.md` | Current Product/UX stable contract and execution state. This Repo Map reflects source through the Step 1A implementation commit; ACTIVE-step state remains owned by runtime. |
...
After Step 1A, the model-presentation boundary is explicitly split: concise current-model summaries use `ModelInfo.current_summary_label`, while combo and diagnostic log paths continue to use the full `display_label`; path detail remains available through tooltips/table. This remains a UI/model-presentation concern and did not cross into controller, engine, transcript-store, download-integrity, or packaging ownership.
