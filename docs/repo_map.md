...
| Branch | `main` |
| Structural baseline | `09425dd3882d35f8c7eb8b8d1aac646e8d40a7d3` (`docs: schedule additional product polish`) |
| Generated | 2026-08-20 |
| Scan scope | 65 Git-tracked files at the baseline; generated and ignored directories were not treated as source |
...
Build wrapper -> Python bootstrap -> whisper.cpp bootstrap -> PyInstaller -> Runtime normalizer
...
| Reproducible source runtime and App packaging | `scripts/`, `packaging/runtime_manifest.json`, PyInstaller specs |
...
| --- | --- | --- | --- |
| `ui_app.py` | PySide6 entry point (`main`), `MainWindow`, `ModelManagerDialog`, `TranscriptTable`, translation lookup, and `EventBridge`. Owns visible controls, transcript/log widgets, dialog lifecycle, Start/Stop requests, and UI refresh. | Uses `TranscriptionController`, model-manager functions, settings constants, transcript parsers, crash logging. It is the PyInstaller entry. | Persists model/beam choices through `AppSettings`; opens Finder with `open`; starts a non-Qt stop thread; receives worker events through a Qt signal. Changes can affect UI responsiveness, controller state gating, model persistence, transcript display, and model-manager tests. |
...
| `settings.py` | Defines transcriber defaults, language normalization/mapping, `TranscriptionSettings`, preflight of CLI/model/beam/task, and config JSON serialization. | Reads path choices from `resource_paths.py` and chunk constants from `stream_transcribe.py`; used by controller, engine, model manager, UI, and tests. | Establishes source-vs-frozen defaults. `TranscriptionSettings.to_config()` becomes session `config.json`; `validate_runtime_paths()` is the Start preflight for executable/model and runtime values. Changes reach command arguments, saved evidence, UI labels, and test fixtures. |
...
| `scripts/build_macos.sh` | Formal Release App build and hard gate. | Validates sources, deletes generated `build/` and `dist/`, runs PyInstaller, normalizes App Runtime, ad-hoc signs, then verifies. | Build success semantics and generated artifacts. `test_packaged_runtime.py`, `test_build_orchestration.py`. |
...
| `scripts/verify_packaged_runtime.py` | Read-only packaged-App verifier. | Checks required files, arm64, RPaths/dependency closure, downloader/model manifest, codesign, bundled CLI smoke, and isolated Runtime smoke. | Formal build and Release ZIP trust boundary; `test_packaged_runtime.py` and `test_release_zip.py`. |
...
| --- | --- | --- |
| `packaging/runtime_manifest.json` | Machine-readable frozen Runtime/build/bundle/release contract plus observed/pending metadata. Enumerates the seven required Runtime components, vendored downloader, model-manifest bundle location, formal build entry, and release filename template. | Consumed by bootstrap helper, PyInstaller specs, Runtime normalizer/verifier, Release ZIP code, and manifest tests. It is the packaging data source of truth. |
...
| `packaging/ClassroomTranscriber.spec` | Release PyInstaller spec; entry is `ui_app.py`, data list is manifest-derived, microphone usage string is declared, and `BUNDLE(..., icon=None)` is current behavior. | Called by Release build; direct future App-icon boundary. `test_packaged_runtime.py`. |
...
| `test_model_manager_ui_contract.py` | `ui_app.py` Model Manager orchestration | AST-level guard that download work remains in the background worker, finished states re-enable/refresh, and UI does not bypass integrity by accepting an existing final file. |
...
| `test_runtime_manifest.py` | `runtime_manifest.json` and related dependency declarations | Manifest schema/portability/no-secret rules, pinned Runtime/build profile, component list, model-integrity packaging linkage, and declared environment alignment. |
...
| `test_packaged_runtime.py` | specs, `build_macos.sh`, package normalizer, packaged verifier | Manifest-driven collection; order of preflight/normalize/sign/verify; missing component/dependency/downloader/manifest failures; architecture and smoke gates. |
| `test_release_zip.py` | `build_release_zip.py` and packaged verifier handoff | Explicit version/locked Python/clean source, ZIP deterministic round-trip, payload exclusions, path and symlink safety, verifier failure handling, and reported digest. |

### Documentation and historical material

| Path / group | Current role in this repository map |
| --- | --- |
| `docs/product_polish_static.md`, `docs/product_polish_runtime.md` | Current Product/UX stable contract and execution state. Step 1A is marked ACTIVE in runtime, but this Repo Map task does not execute that implementation step. |
...
Model selection has two UI surfaces: the main combo calls `MainWindow._set_selected_model()`, while `ModelManagerDialog._select_model()` persists its selection then emits `model_selected` to the MainWindow. Scans/refreshes update presentation and may restore a persisted selected model; they are not a separate user-success event.
...
resource_paths.py
  ...
  -> settings.py module defaults
     -> model_manager.AppSettings JSON (selected model, CLI, beam, model roots/imports/download root)
     -> TranscriptionSettings for a newly started session
        -> config.json in that session
```

`AppSettings` persists model-related selection and default beam at `APP_SETTINGS_PATH`. It does **not** currently persist a UI-language selection or configurable output base/root. `DEFAULT_UI_LANGUAGE` comes from the process environment and `TEXT` lookup; `Original Language` maps independently to the whisper CLI language code. `TranscriptionSettings.output_root` is currently the resolved actual `outputs` directory: source mode defaults under the project, frozen mode under the user Documents application directory.

Current `validate_runtime_paths()` preflights beam/language/task, `whisper-cli`, and selected model before creating a session. A separate preflight for a user-configured/writable output root does not exist in current source, because configurable output root is still pending Product Polish work.
...
-> scripts/bootstrap_and_build.sh
   ...
   -> PYTHON=.venv/bin/python scripts/build_macos.sh
      ...
      -> PyInstaller Release spec (ui_app.py + manifest data)
...
The ignored `external/whisper.cpp` is a build input reconstructed by bootstrap, not a tracked source dependency to copy. The resulting App bundles CLI/dylibs, the vendored downloader, and model-integrity manifest; it deliberately does not bundle model binaries or user settings.
...
| --- | --- | --- | --- |
| **1A Current-model readability** | `ui_app.py` current label/combo/dialog summary; `model_manager.py` `ModelInfo` display properties; UI/model tests. | `display_label` is shared by main combo, summaries, and logs; full model path is already a MainWindow tooltip and Model Manager has an independent Path table column. Model availability/selectability must remain unchanged. | `test_ui_support.py`, `test_model_manager_ui_contract.py`, model discovery/selection smoke. |
| **1B Model selection transient confirmation** | `ModelManagerDialog` selection signal and `MainWindow` selection/refresh paths; `AppSettings` persistence. | Selection, refresh/scan, startup restoration, unavailable rows, and download-completion selection are distinct flows. UI feedback must not change controller-selected model or settings semantics. | Model UI contract tests, model support tests, targeted Qt interaction smoke. |
| **1C Download visible progress/busy feedback** | `ModelManagerDialog` download state, background worker, Qt completion signal; model manager/integrity modules. | Network download and SHA validation remain off the Qt main thread; only verification may yield an available/selectable official model; close/retry state must remain safe. | `test_model_integrity.py`, `test_model_manager_ui_contract.py`, `test_model_download_resources.py`, targeted success/failure dialog-state smoke. |
| **1D Configurable output root** | `resource_paths.py`, `settings.py`, `model_manager.AppSettings` or equivalent persistent-settings boundary, controller start, UI; `TranscriptStore` as evidence-contract reference. | Product setting is a base/root while store currently expects the actual `outputs` directory. Preserve `<base>/outputs/<timestamp>` and do not alter historical sessions, evidence filenames, raw/clean semantics, or Start failure ordering. | `test_ui_support.py` path/config/store/Stop coverage; new targeted settings/controller/session-path cases; packaged smoke if frozen paths change. |
| **1E Chinese / English UI switch and semantic alignment** | `settings.py` UI language constants/normalization; `ui_app.py` `TEXT`, `tr`, and widget construction; model dialog strings. | UI language is distinct from `Original Language` and whisper command `-l` mapping. Existing UI language is process-default based, not persisted app selection. | Language mapping/command assertions in `test_ui_support.py`, model-manager UI checks, targeted bilingual UI smoke, normal transcription regression. |
| **1F Packaged App icon** | Release spec, any future tracked icon asset, formal build path, Runtime/release verification boundary. | Current Release spec has `icon=None`; no final asset or visual design is established. Packaging must stay repo-relative and ZIP round-trip safe. | `test_packaged_runtime.py`, `test_release_zip.py`, formal App build/verifier, Finder/Dock/ZIP acceptance after asset choice. |
...
For current **1A**, the closest architecture boundary is the separation between the shared `ModelInfo.display_label` presentation value and the individual consumers in the main summary, Model Manager summary/table, combo, and log paths. This is a UI/model-presentation boundary; it should not require crossing into controller, engine, transcript-store, download-integrity, or packaging ownership.
...
3. The historical Deployment UX backlog says the current-model path cannot be accessed in full. Current `ui_app.py` sets a full-path tooltip on the MainWindow current-model label; the Model Manager table independently exposes Path. The Product Polish runtime's more precise description matches current source better.
4. Product Polish 1D's default user-selected base/root is a future contract. Current source still uses the project `outputs/` directory in source mode and the user Documents application directory in frozen mode; it has no configurable output-root setting yet. This is an intentional pending feature, not evidence that the future contract is implemented.
5. The legacy `run_long_test.sh` contains a machine-specific path, unlike the formal build scripts' repo-relative design. It is not part of the formal developer-build or Release path.

## Unknown / not established by the static scan

- This scan does not independently reproduce microphone, Metal, or ordinary-user acceptance; the PASS claims are recorded in deployment runtime evidence, not re-established by code reading.
- The future App icon's asset, visual direction, and packaging representation are explicitly undecided; no tracked formal icon asset exists at this baseline.
- No tracked module implements the planned LLM sidecar, session browser, persistent backend, or configurable output root/UI-language switch; planning documents are not implementation evidence.
