...
| Branch | `main` |
| Structural baseline | `4d2059d273d0ba94373b1969b8236ef462b6acae` (`fix: add model selection confirmation`) |
| Updated | 2026-08-21 |
...
| --- | --- | --- | --- |
| `ui_app.py` | PySide6 entry point (`main`), `MainWindow`, `ModelManagerDialog`, `TranscriptTable`, translation lookup, and `EventBridge`. Owns visible controls, transcript/log widgets, dialog lifecycle, Start/Stop requests, UI refresh, and the Step 1B transient model-selection feedback surfaces. | Uses `TranscriptionController`, model-manager functions, settings constants, transcript parsers, crash logging. It is the PyInstaller entry. | Persists model/beam choices through `AppSettings`; opens Finder with `open`; starts a non-Qt stop thread; receives worker events through a Qt signal. MainWindow and ModelManagerDialog each own a reusable single-shot 2 s `QTimer` plus transient confirmation `QLabel`; only explicit changed/available user selection shows confirmation. Changes can affect UI responsiveness, controller state gating, model persistence, transcript display, and model-manager tests. |
...
| `test_model_manager_ui_contract.py` | `ui_app.py` Model Manager orchestration and model presentation/selection-feedback contracts | Guards that current summaries stay concise with full-path tooltips, combo/log/table keep full presentation where intended, explicit changed/available selection alone triggers transient bilingual non-modal confirmation, the confirmation uses parent-owned single-shot 2 s timers and auto-clear wiring, automatic restore/import/download paths do not confirm, and download work/integrity boundaries remain unchanged. |
...
- No tracked module implements the planned LLM sidecar, session browser, persistent backend, configurable output root, or UI-language switch at the Step 1B structural baseline; planning documents are not implementation evidence.
